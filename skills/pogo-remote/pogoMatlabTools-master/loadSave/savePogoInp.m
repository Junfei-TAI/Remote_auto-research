...
% model.globalDof - are dofs defined in global numbering (1) or local (0)
% model.nDofPerNode - unused in latest version
...
%
% model.dofScales - scales associated with the degrees of freedom - used
...
% Updated v 1.16 (change Dof definiton), July 2024, PH
% Updated v 1.17 (added dofScales), Oct 2024, PH
% Updated to prevent saving multiple measurement sets with the same name, Aug 2025, PH
        ...
        if fileMajVer == 1 && (fileMinVer == 11 || fileMinVer == 12)
            ...
            for c = 1:metadataNumValues
                ...
                if (length(getfield(model.metadata,mdNames{c})) ~= 1)
                    ...
                    %continue;
                    warning('Unable to store metadata arrays in file format %d, \nso only storing first value', fileVerFull)
                    arrVals = getfield(model.metadata,mdNames{c});
                    model.metadata = setfield(model.metadata,mdNames{c},arrVals(1));
                end
...
if fileVerFull >= 1016
    ...
    if ~isfield(model,'globalDof')
        model.globalDof = 1;
        warning('Model set to use global degrees of freedom. To use traditional ordering set model.globalDof to 0.')
    end
    if model.globalDof == 0
        warning('model.globalDof is set to 0. This is not recommended and unlikely to be supported in future, but should work for now.')
    end
    if model.globalDof > 1
        error('model.globalDof field is set to %d, must be in range 0-1.', model.globalDof)
    end

    fwrite(fid, model.globalDof, 'int32');

    if ~isfield(model,'nDofPerNode')
        model.nDofPerNode = 0;
    end

    fn = remFieldName(fn,'globalDof');
end
...
if ~isfield(model,'notes')
    model.notes = zeros(1,1024);
end
model.notes = char(model.notes);
...
for eCnt = 1:model.nElTypes
    ...
    elType = char(model.elTypes{eCnt}.name);
...
if ~isfield(model,'fixNodes')
    ...
    if min(d(:)) <= 0 || max(d(:)) > 16 %max could be updated but should be <= 16 for hard wired dofs
        error('Range error on model.fixDof. Should be 1 indexed DOF values')
    end
    if fileVerFull >= 1016
        model.fixDofD = (n(:)-1) * 16 + d(:)-1;
    else
...
if fileVerFull >= 1008
    if ~isfield(model,'dofGroups')
        ...
        for dCnt = 1:nDofGroups
            ...
            if fileVerFull >= 1016
                dofSpec = uint64(model.dofGroups{dCnt}.dofSpec(:))-1 + (uint64(model.dofGroups{dCnt}.nodeSpec(:))-1)*16;
            elseif fileVerFull >= 1010
            ...
            if ~isequal(size(model.dofGroups{dCnt}.dofWeight), size(model.dofGroups{dCnt}.nodeSpec))
                size(model.dofGroups{dCnt}.dofWeight)
                size(model.dofGroups{dCnt}.nodeSpec)
                error('dofGroups{%d} .dofWeight and .nodeSpec have mismatching sizes ', dCnt)
            end
...
for fCnt = 1:nShots
    ...
    for sCnt = 1:nSigs
        ...
        if (fileVerFull >= 1008)  && isDofGroup == 0
        ...
        if isDofGroup == 0
            if max(s.nodeSpec) > model.nNodes || min(s.nodeSpec) < 1
                error('Range error on model.shots{%d}.sigs{%d}.nodeSpec: %d',fCnt,sCnt,s.nodeSpec)
            end
            if max(s.dofSpec) > 16 || min(s.dofSpec) < 1
                error('Range error on model.shots{%d}.sigs{%d}.dofSpec: %d',fCnt,sCnt,s.nodeSpec)
            end

            if fileVerFull >= 1016
                dofSpec = (uint64(s.nodeSpec(:)) - 1)*16 + (uint64(s.dofSpec(:)) - 1);
            elseif fileVerFull >= 1010
        ...
        if length(s.sig(:)) ~= f.ntSig
            error('model.shots{%d}.sigs{%d}.sig must be a vector.',fCnt,sCnt)
        end
...
for mSetCnt = 1:nMeasSets
    ...
    model.measSets{mSetCnt}.name = char(model.measSets{mSetCnt}.name);

    %check the name hasn't been used before...
    for mSetCnt2 = 1:mSetCnt-1
        if strcmp(char(model.measSets{mSetCnt}.name), char(model.measSets{mSetCnt2}.name))
            error('model.measSets{mSetCnt}.name "%s" is not unique. \nMeasurement sets must have different names.',model.measSets{mSetCnt}.name)
        end
    end
    ...
    if isDofGroup == 0
        ...
        if ~isfield(model.measSets{mSetCnt},'measDof')
            error('model.measSets{mSetCnt}.measDof missing')
            % better to do this than error elsewhere
        ...
        if nMeas > 0
            ...
            if max(model.measSets{mSetCnt}.measDof(:)) > 16 || min(model.measSets{mSetCnt}.measDof(:)) < 1
                error('Range error on model.measDof.')
            end

            if fileVerFull >= 1016
                measDof = (uint64(model.measSets{mSetCnt}.measNodes(:))-1)*16 + (uint64(model.measSets{mSetCnt}.measDof(:))-1);
            elseif fileVerFull >= 1010
...
%dofScales
if fileVerFull >= 1017
    fwrite(fid, stringSaveTidy('dofScale',40), '*char');
    fwrite(fid, 1, 'int32');
    ...
    if ~isfield(model,'dofScales')
        fwrite(fid, 0, 'uint32');
    else
        % nDofScales = sum(model.dofScales >= 0);
        nDofScales = length(model.dofScales);
        fwrite(fid, nDofScales, 'uint32');

        for cnt = 1:nDofScales
            fwrite(fid, model.dofScales(cnt), precString);
        end

        fn = remFieldName(fn,'dofScales');
    end


    %end of dofScale section
    %go back and put the section size in at the beginning
    curPos = ftell(fid);
    sectSize = curPos - startPos;
    fseek(fid, writePos,-1);
